...
import { Customer, CommercialBank, ExchangeBank } from "../instances";
import { System, systemCheck } from "../systems";
import { CustomerService, BankService, ExchangeBankService } from "../services";
...
describe("set up correspondent system", () => {
  ...
  it("has a nostro and vostro list in balances", () => {
    const bank1 = new ExchangeBank("bank1", "ecus");
    expect(Object.keys(bank1.accounts)).toEqual(["nostro", "vostro"]);
  });
  it("should have correct currency for each bank", () => {
    const bank1 = new ExchangeBank("bank1", "ecus");
    const bank2 = new ExchangeBank("bank2", "lyonaise");
    expect(bank1.domesticCurrency).toEqual("ecus");
    expect(bank2.domesticCurrency).toEqual("lyonaise");
  });
  it("should have correct corresponding account between two banks", () => {
    const bank1 = new ExchangeBank("bank1", "ecus");
    const bank2 = new ExchangeBank("bank2", "lyonaise");
    ExchangeBankService.createCorrespondence(bank1, bank2, 1000);
    expect(bank1.accounts.nostro[0]).toEqual({
      id: "bank2",
      type: "bankDeposits",
      amount: 1000,
    });
    expect(bank1.accounts.vostro[0]).toEqual({
      id: "bank2",
      type: "bankDeposits",
      amount: 1000,
    });
    expect(bank2.accounts.nostro[0]).toEqual({
      id: "bank1",
      type: "bankDeposits",
      amount: 1000,
    });
    expect(bank2.accounts.vostro[0]).toEqual({
      id: "bank1",
      type: "bankDeposits",
      amount: 1000,
    });
